function [topicPub,messagePub] = onlineAlgoTest_02(topicSub,messageSub,password)
...
persistent tElapsedOpeningRequest;
persistent nFile
    ...
    elseif close
        ...
        elseif StatusClose == -1 || price < 0
            ...
            if trialClose < 5
                ...
                [topicPub,messagePub,startingOperation] = onlineClose(lastCloseValue,operLots,operCloseSlippage,ticket,algoTopicPub,algoMagic,indexClose);
...
if strcmp(ms.machineStatus,'opening')
    ...
    tElapsedOpeningRequest = toc(tStartOpeningRequest);
    
    if tElapsedOpeningRequest > 300
        
        LogObj.error('error','no Status message received for opening the requested position');
...
if abs(updatedOperation) > 0 && startingOperation == 0
    ...
    [topicPub,messagePub,startingOperation] = onlineOpen(oper,openValue,operLots,operOpenSlippage,MT4stopL,MT4takeP,algoTopicPub,algoMagic,indexOpen);
...
elseif updatedOperation == 0 && abs(startingOperation) > 0
    % Closing request
    [topicPub,messagePub,startingOperation] = onlineClose(closeValue,operLots,operCloseSlippage,ticket,algoTopicPub,algoMagic,indexClose);
