function [topicPub,messagePub] = onlineAlgoTest_client01(topicSub,messageSub,password)
...
elseif listener3 && ( strcmp(ms.machineStatus,'closing') || strcmp(ms.machineStatus,'opening') )%new status
    ...
    elseif close
        ...
        elseif StatusClose == -1 || price < 0
            ...
            if trial < 5
                trial=trial+1;
                [topicPub,messagePub,startingOperation]=onlineCloseTest(lastCloseValue,ticket,indexClose);
...
if abs(updatedOperation) > 0 && startingOperation == 0
    ...
    [topicPub,messagePub,startingOperation]=onlineOpenTest(oper,openValue,MT4stopL,MT4takeP,indexOpen);
...
elseif updatedOperation == 0 && abs(startingOperation) > 0
    % Closing request
    [topicPub,messagePub,startingOperation]=onlineCloseTest(closeValue,ticket,indexClose);
