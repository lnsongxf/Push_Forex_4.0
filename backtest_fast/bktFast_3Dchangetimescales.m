classdef bktFast_3Dchangetimescales < handle
    ...
    methods
        ...
        function [obj] = optimize(obj,parameters)
            ...
            for nts=1:length(newTimeScale)
                
                [hisData, newHisData] = load_historical(histName, actTimeScale, newTimeScale(nts));
                ...
                if ( exist('reverse_optimization','var') && reverse_optimization == 1 )
                    ...
                    rnTraining = floor(rTraining/newTimeScale(nts));
                    ...
                    %             skipMe = floor(r*0.20);
                    %             skipMeNewTime = floor(skipMe/newTimeScale(nts));
                ...
                for n = N
                    ...
                    for m = M
                        ...
                        bktfast = bktfast.spin(hisDataTraining(:,4), newHisDataTraining, actTimeScale, newTimeScale(nts), n, m, transCost, pips_TP, pips_SL, stdev_TP, stdev_SL, 0);
                        
                        % if there are enough operations then save the stats
                        if bktfast.indexClose>20
                            
                            p = Performance_06;
                            performance = p.calcSinglePerformance(nameAlgo,'bktWeb',histName,Cross,newTimeScale(nts),transCost,10000,10,bktfast.outputbkt,0);
                    ...
                    if max(obj.R_over_maxDD(n,:)) > 0.8
                        ...
                        temp_Training = temp_Training.spin(hisDataTraining(:,4), newHisDataTraining, actTimeScale, newTimeScale(nts), n, ind_best, transCost, pips_TP, pips_SL, stdev_TP, stdev_SL, 0);
                        
                        if temp_Training.indexClose>20
                            performance_temp_Training = p.calcSinglePerformance(nameAlgo,'bktWeb',histName,Cross,newTimeScale(nts),transCost,10000,10,temp_Training.outputbkt,0);
                            
                            display(['Train: best R/maxDD=' , num2str(current_best),'. Avg pips/operat= ',num2str(performance_temp_Training.pipsEarned/temp_Training.indexClose),'.  N =', num2str(n),' M =', num2str(ind_best),' timescale =', num2str(newTimeScale(nts)) ]);
                            ...
                            temp_paperTrad = temp_paperTrad.spin(hisDataPaperTrad(:,4), newHisDataPaperTrad, actTimeScale, newTimeScale(nts), n, ind_best, transCost, pips_TP, pips_SL, stdev_TP, stdev_SL, 0);
                            performance_temp = p.calcSinglePerformance(nameAlgo,'bktWeb',histName,Cross,newTimeScale(nts),transCost,10000,10,temp_paperTrad.outputbkt,0);
                ...
                display(['bestN =', num2str(bestN),' bestM =', num2str(bestM),' timescale =', num2str(newTimeScale(nts))]);
                
                obj.bktfastTraining = feval(algo);
                obj.bktfastTraining = obj.bktfastTraining.spin(hisDataTraining(:,4), newHisDataTraining, actTimeScale, newTimeScale(nts), bestN, bestM, transCost, pips_TP, pips_SL, stdev_TP, stdev_SL, 0);
                
                p = Performance_06;
                obj.performanceTraining = p.calcSinglePerformance(nameAlgo,'bktWeb',histName,Cross,newTimeScale(nts),transCost,10000,10,obj.bktfastTraining.outputbkt,0);
                ...
                obj.bktfastPaperTrading = obj.bktfastPaperTrading.spin(hisDataPaperTrad(:,4), newHisDataPaperTrad, actTimeScale, newTimeScale(nts), bestN, bestM, transCost, pips_TP, pips_SL, stdev_TP, stdev_SL, 0);
                
                obj.performancePaperTrad = p.calcSinglePerformance(nameAlgo,'bktWeb',histName,Cross,newTimeScale(nts),transCost,10000,10,obj.bktfastPaperTrading.outputbkt,0);
