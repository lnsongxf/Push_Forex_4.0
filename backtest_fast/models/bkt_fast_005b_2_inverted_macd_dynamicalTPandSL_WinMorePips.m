classdef bkt_fast_005b_2_inverted_macd_dynamicalTPandSL_WinMorePips < handle
    ...
    methods
        ...
        function obj = spin(obj, Pminute, matrixNewHisData, ~, newTimeScale, N, dynclose, cost, ~, ~, wTP, wSL, plottami)
            
            % Pminute = prezzo al minuto
            % P = prezzo alla new time scale
            % date = data alla new time scale
            % N = smooth per calcolare la volatility
            % dynclose = variabile dinamica della funzione di chiusura
            ...
            while i < sizeStorico
                ...
                if  ( abs( s(i) - s(i-1) ) == 2 )
                    ...
                    for j = newTimeScale*(i):length(Pminute)
                        ...
                        dynamicParameters {1} = dynclose;
                        dynamicParameters {2} = dynclose;
