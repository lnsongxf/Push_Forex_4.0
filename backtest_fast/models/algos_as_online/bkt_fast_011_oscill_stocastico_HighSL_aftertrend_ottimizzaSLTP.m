classdef bkt_fast_011_oscill_stocastico_HighSL_aftertrend_ottimizzaSLTP < handle
    ...
    methods
        ...
        function obj = spin(obj, Pminute, matrixNewHisData, ~, newTimeScale, kperiods, initialTPSL, cost, ~, ~, wTP, wSL, plottami)
            ...
            %%% QUESTO LO USO PER OTTIMIZZARE I VALORI INIZIALI DI TP E SL,
            %%% ACCETTANDO CHE I PARAMETRI MIGLIORI SONO 3(freq oscillatore) e 81(lateSL) PER AUDCAD
            ...
            while i <= sizeStorico
                ...
                if  ( abs( s(i-1) ) && ( s(i)==0 ) && ( ( P(i-5) - P(i) ) / P(i) * s(i-1) > 0.002 ) )
                    ...
                    ntrades = ntrades + 1;
                    [obj, Pbuy, ~] = obj.apri(i, P, cost, ntrades, segnoOperazione, date);
%                      display(['Pbuy =', num2str(Pbuy), ' segno =', num2str(segnoOperazione)]);
                    TakeP = initialTPSL; % min(floor(wTP*devFluct2),100);
                    StopL = initialTPSL; % min(floor(wSL*devFluct2),100);
