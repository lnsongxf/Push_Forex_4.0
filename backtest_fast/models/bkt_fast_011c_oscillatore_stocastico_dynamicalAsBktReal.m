classdef bkt_fast_011c_oscillatore_stocastico_dynamicalAsBktReal < handle
    ...
    methods
        ...
        function obj = spin(obj, Pminute, matrixNewHisData, ~, newTimeScale, kperiods, dperiods, cost, ~, ~, wTP, wSL, plottami)
                ...
                if  ( abs( s(i-1) ) && s(i)==0 )
                    ...
                    for j = newTimeScale*(i):length(Pminute)
                        ...
                        if ( condTP >=0 ) || ( condSL >= 0 )
                            ...
                            i = indice_I;
                            obj.chei(ntrades)=i;
                            obj.obj.indexClose = obj.obj.indexClose + 1;
                            break
            ...
            obj.outputbkt(:,1) = obj.chei(1:obj.obj.indexClose);                    % index of stick
            obj.outputbkt(:,2) = obj.openingPrices(1:obj.obj.indexClose);      % opening price
            obj.outputbkt(:,3) = obj.closingPrices(1:obj.obj.indexClose);        % closing price
            obj.outputbkt(:,4) = (obj.closingPrices(1:obj.obj.indexClose) - ...
                obj.openingPrices(1:obj.obj.indexClose)).*obj.direction(1:obj.obj.indexClose);   % returns
            obj.outputbkt(:,5) = obj.direction(1:obj.obj.indexClose);              % direction
            obj.outputbkt(:,6) = ones(obj.obj.indexClose,1);                    % real
            obj.outputbkt(:,7) = obj.OpDates(1:obj.obj.indexClose);              % opening date in day to convert use: d2=datestr(outputDemo(:,2), 'mm/dd/yyyy HH:MM')
            obj.outputbkt(:,8) = obj.ClDates(1:obj.obj.indexClose);                % closing date in day to convert use: d2=datestr(outputDemo(:,2), 'mm/dd/yyyy HH:MM')
            obj.outputbkt(:,9) = ones(obj.obj.indexClose,1)*1;                 % lots setted for single operation
