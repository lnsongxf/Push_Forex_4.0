...
if(isempty(countCycle) || countCycle == 0)
    ...
    params = Parameters;
    map('Algo_005b_02') = RealAlgo(operationState,params);
    oper = 0;
    return;
end
...
if (newTimeScalePoint && abs(operationState.actualOperation) == 0)
    ...
    % 01c
    % -------- coreState filter -------------------- %
    cState.core_Algo_005b_inverted_macd(chiusure(1:end-1),20);
...
if operationState.lock
    ...
    if abs(operationState.actualOperation) > 0 && newTimeScalePoint == 0 ;
        ...
        if openValueReal > 0
            ...
            dynamicParameters {2} = 6;
            [params,TakeProfitPrice,StopLossPrice,dynamicOn] = dynamicalTPandSLManager(operationState, chiusure, params, @closingWinMorePips, dynamicParameters);
            if dynamicOn  == 1
