function [oper, openValue, closeValue, stopLoss, noLoose, minReturn] = Algo100000_test(matrix,newTimeScalePoint,newTimeScalePointEnd,openValueReal,timeSeriesProperties,indexHisData)
...
if(isempty(countCycle) || countCycle == 0)
    ...
    params = Parameters;
    map('Algo_000_test') = RealAlgo(operationState,params);
    oper = 0;
    return;
end


ra = map('Algo_000_test');
remove(map,'Algo_000_test');
...
if newTimeScalePoint
    ...
    % 01c
    % -------- coreState filter -------------------- %
    cState.core_Algo_000_test( chiusure(1:end-1) );
