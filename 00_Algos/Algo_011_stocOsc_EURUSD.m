function [oper, openValue, closeValue, stopLoss, takeProfit, minReturn] = Algo_011_stocOsc_EURUSD(matrix,newTimeScalePoint,newTimeScalePointEnd,openValueReal,timeSeriesProperties,indexHisData)
...
if(isempty(countCycle) || countCycle == 0)
    ...
    params = Parameters;
    map('Algo_011_stocOsc_EURUSD') = RealAlgo(operationState,params);
    oper      = 0;
    return;
end


ra = map('Algo_011_stocOsc_EURUSD');
remove(map,'Algo_011_stocOsc_EURUSD');
...
if newTimeScalePoint
    ...
    % 01c
    % -------- coreState filter -------------------- %
    cState.core_Algo_011_stocOsc( lows(1:end-1), highs(1:end-1), chiusure(1:end-1), params ,1,1);
...
map('Algo_011_stocOsc_EURUSD')     = real_Algo;
