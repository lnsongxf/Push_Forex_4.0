function [topicPub,messagePub] = onlineAlgoLore2(topicSub,messageSub)
...
listener1 = strcmp(topicSub,'TIMEFRAMEQUOTE@MT4@ACTIVTRADES@EURUSD@m1@v40');
...
%      display( strcat('Type: ', type) );
    ...
    elseif close
        ...
        elseif StatusClose == -1
            ...
            trial=trial+1;
            [topicPub,messagePub,startingOperation]=onlineCloseLore(price,ticket,indexClose);
            display( strcat('Matlab trial #',num2str(trial),' to close the operation ',num2str(ticket)) );
            %pause(10); % provo a metter una pausa
...
if abs(updatedOperation) > 0 && startingOperation == 0
    ...
    % Opening request
    [topicPub,messagePub,startingOperation]=onlineOpenLore(oper,openValue,50,50,indexOpen);
    
    
elseif updatedOperation == 0 && abs(startingOperation) > 0
    % Closing request
    [topicPub,messagePub,startingOperation]=onlineCloseLore(closeValue,ticket,indexClose);
