...
function compile(flags, file, outputdir)
  ...
  if isoctave
    ...
    fileIdPub = fopen('configCompile.txt');
    ListP = textscan(fileIdPub,'%s');
    srcPath = ListP{1}{1};
    C_split = strsplit( srcPath,';');
    
    compileSrcPath = C_split{1}
    compileIncludePath = C_split{2}
    compileLibPath = C_split{3}
    compileLibZmq = C_split{4}
    
    %flags = { ['-I' 'C:\code\Push_Forex_4.0\nodeM\MatlabZeroMQ\matlab-zmq\src'], ['-I' 'C:\PROGRA~1\ZEROMQ~1.4\include'],['-L' 'C:\PROGRA~1\ZEROMQ~1.4\lib\'],['-l' 'libzmq-v120-mt-4_0_4']};
    flags = { ['-I' compileSrcPath], ['-I' compileIncludePath],['-L' compileLibPath],['-l' compileLibZmq]};
