function [topicPub,messagePub,startingOperation]=onlineOpenAle(oper,openValue,stopLoss,takeProfit,indexOpen)
...
magic=strcat('magic=',num2str(1000));                                       % magic number, algo id.
sl=strcat('sl=',num2str(stopLoss));                                        % not madatory
tp=strcat('tp=',num2str(takeProfit));                                      % not mandatory
topicPub='OPERATIONS@ACTIVTRADES@EURUSD@1000';
