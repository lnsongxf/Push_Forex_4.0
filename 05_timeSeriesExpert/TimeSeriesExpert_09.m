classdef TimeSeriesExpert9 < handle
    ...
    methods
        ...
        function plotBinary(obj,newTimeScale)
            ...
            dataClosureRescaled=obj.closeVrescaled;
            
            totDataMinutes=length(dataClosure);

            figure(100);
            %plot(dataClosure,'-b');
            l=(s:s:length(dataClosureRescaled)*s);
            plot(l,dataClosureRescaled,'-b');
            ...
            xi=obj.startMacroTrend.*s;
            d=diff(xi);
            lengthTrends=[d; totDataMinutes-xi(end)];
            xfin=xi+lengthTrends;
        ...
        function [obj]=maxMinSearch(obj,type,rate,t0s,i,x1,nRescale,yEnd_f1)
            
            %%% initialize
            
            r=type*rate;
            price=obj.closeV;
            %price=obj.closeVrescaled;
...
%             if i-t0s(end)<5
%                 display(i)
%                 obj.lastM=last*nRescale;
%             end

            obj.lastM=last;
            index1=[index1;obj.lastM];
            %index1=[index1;last];
            %display(index1);
            %display(index1);
            %[~,~,index2] = find(w);
        ...
        function [type,rate,err,indexStart,i]=linFitTrendRec(obj,data,actTimeScale,newTimeScale,n,fun,inFit)
            ...
            nRescale=newTimeScale/actTimeScale;
            %oldL=length(obj.closeV);
            %newL=floor(oldL/nRescale);
            %xtot=(nRescale:nRescale:newL*nRescale)';
            ...
            while tf==0
                ...
                %x1=(t01:nRescale:i*nRescale)';
                x1=(x01:nRescale:i*nRescale)';
                ct1=c(t01:1:i);
                [vEnd_f1, yEnd_f1, err1] = fit1(n,x1,ct1,fun,inFit1);
                if x1(end)-x1(1)<5*nRescale
                    err1=1;
                end
                err(i,1)=err1;
...
%                 err(i,1)=err1;
                ...
                if i>t01+2;
                    ...
                    [obj]=maxMinSearch(obj,type(in,1),rate(in,1),t0s,i,x1,nRescale,yEnd_f1);
                    ...
                    if length(obj.positionMax)>1
                        t0M=obj.positionMax;
                        closurePrice=c(t0M);
                        [vEup, yFup] = fitlsq(n,obj.positionMax,closurePrice,fun,inFitBinary,vin,vfin,m0);
                        %display(vEup);
                        q0up(in,1)=vEup(1,2);
